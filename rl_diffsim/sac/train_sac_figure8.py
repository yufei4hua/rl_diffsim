"""SAC training script for reach position task with rotor velocity control.

Soft Actor-Critic (SAC) implementation for quadrotor position control using direct RPM setpoints.
Features:
...
from rl_diffsim.sac.sac_agent import SACAgent
...
class Args:
    ...
    jax_device: str = "gpu"
    """environment device"""
    exp_name: str = "sac_rprv"
    """the name of the experiment"""
    wandb_project_name: str = "rl-sac-rprv"
    ...
    alpha_lr: float = 3e-4
    """the learning rate of the temperature (alpha) optimizer"""
    gamma: float = 0.99
    """the discount factor gamma"""
    tau: float = 0.005
    """target network update rate (Polyak averaging)"""
    init_alpha: float = 1.0
    """initial temperature (alpha) value"""
    autotune_alpha: bool = True
    """whether to automatically tune temperature (alpha)"""
    ...
    init_logstd: tuple = (-1.0, -1.0, -1.0, -1.0)
    """initial log standard deviation for actor"""
    ...
    rpy_coef: float = 0.2
    act_coefs: tuple = (0.075,) * 4
    d_act_coefs: tuple = (0.1,) * 4
...
def make_jitted_envs(num_envs: int, jax_device: str, args: Args, reset_rotor: bool = True) -> ReachPosEnv:
    """Create wrapped environments for SAC training."""
...
def collect_rollout(
    ...
    """Collect rollout data using stochastic policy and add to buffer.
    ...
    def step_once(
        ...
        if random_action:
            ...
            # SAC: sample from stochastic policy
            action, _, key = agent.get_action_sample(agent.actor_state.params, obs, key)
...
def update_policy(
    args: Args, agent: SACAgent, buffer: ReplayBuffer, key: Array
) -> tuple[SACAgent, Array, Array, Array, Array]:
    """Perform SAC policy updates with critic, actor, and alpha updates.

    Returns:
        (agent, critic_loss, actor_loss, alpha_loss, key)
    """
    ...
    # Alpha optimizer for temperature tuning
    alpha_optimizer = optax.adam(learning_rate=args.alpha_lr)

    def update_step(
        carry: tuple[SACAgent, Array], idx: int
    ) -> tuple[tuple[SACAgent, Array], tuple[Array, Array, Array]]:
        agent, key = carry
        ...
        # Current alpha
        alpha = jp.exp(agent.log_alpha)

        # === Critic Update ===
        # Sample next actions from current policy (no target actor in SAC)
        next_actions, next_log_probs, key = agent.get_action_and_logprob(
            agent.actor_state.params, next_obs, key
        )

        # Compute target Q with entropy bonus
        target_q1 = agent.get_q(agent.target_critic1_params, next_obs, next_actions)
        target_q2 = agent.get_q(agent.target_critic2_params, next_obs, next_actions)
        min_target_q = jp.minimum(target_q1, target_q2)

        # SAC target: r + γ * (1 - d) * (min_Q' - α * log_prob')
        target_q = rewards + args.gamma * (1.0 - dones.astype(jp.float32)) * (
        ...
        # === Actor Update ===
        # SAC actor loss: E[α * log_prob - Q(s, a)]
        def actor_loss_fn(params: dict[str, Any]) -> tuple[Array, Array]:
            sampled_actions, log_probs, _ = agent.get_action_and_logprob(params, obs, key)
            q1 = agent.get_q(critic1_state.params, obs, sampled_actions)
            q2 = agent.get_q(critic2_state.params, obs, sampled_actions)
            min_q = jp.minimum(q1, q2)
            actor_loss = jp.mean(alpha * log_probs - min_q)
            return actor_loss, log_probs

        (actor_loss, log_probs), a_grads = jax.value_and_grad(actor_loss_fn, has_aux=True)(
        ...
        def alpha_loss_fn(log_alpha: Array) -> Array:
            # Loss = -α * E[log_prob + target_entropy]
            return -jp.mean(jp.exp(log_alpha) * (log_probs.mean() + agent.target_entropy))

        if args.autotune_alpha:
            alpha_loss, alpha_grads = jax.value_and_grad(alpha_loss_fn)(agent.log_alpha)
            alpha_updates, new_alpha_opt_state = alpha_optimizer.update(
                alpha_grads, agent.alpha_optimizer_state
            )
            new_log_alpha = optax.apply_updates(agent.log_alpha, alpha_updates)
        else:
            alpha_loss = jp.array(0.0)
            new_log_alpha = agent.log_alpha
            new_alpha_opt_state = agent.alpha_optimizer_state

        # === Soft update target critics ===
        new_target_critic1_params = optax.incremental_update(
            critic1_state.params, agent.target_critic1_params, args.tau
        )
        new_target_critic2_params = optax.incremental_update(
            critic2_state.params, agent.target_critic2_params, args.tau
        )
        ...
        return (agent, key), (critic_loss, actor_loss, alpha_loss)

    (agent, key), (critic_losses, actor_losses, alpha_losses) = lax.scan(
        update_step, (agent, key), jp.arange(args.updates_epochs)
    )

    mean_critic_loss = jp.mean(critic_losses)
    mean_actor_loss = jp.mean(actor_losses)
    mean_alpha_loss = jp.mean(alpha_losses)

    return agent, mean_critic_loss, mean_actor_loss, mean_alpha_loss, key


# region train_sac
def train_sac(args: Args, model_path: Path, jax_device: str, wandb_enabled: bool = False) -> tuple:
    """Train SAC agent."""
    ...
    agent = SACAgent.create(
        ...
        init_logstd=jp.array(args.init_logstd),
    )

    print(f"Initial alpha: {agent.alpha}, target_entropy: {agent.target_entropy}")
    ...
    def train_iteration(carry: tuple, _) -> tuple[tuple, tuple]:
        ...
        # 1. Collect rollout with stochastic policy
        ...
        # 2. Update policy (critic + actor + alpha)
        agent, critic_loss, actor_loss, alpha_loss, key = update_policy(args, agent, buffer, key)

        return (envs, agent, buffer, obs, done, sum_rewards, key), (
            rollout_data,
            critic_loss,
            actor_loss,
            alpha_loss,
    ...
    (
        ...
        (all_rollout_data, all_critic_loss, all_actor_loss, all_alpha_loss, all_alpha),
    ...
    print(f"Final alpha: {agent.alpha}")
                ...
                wandb.log(
                    {
                        ...
                        "losses/alpha_loss": float(all_alpha_loss[iter_idx]),
                        "losses/alpha": float(all_alpha[iter_idx]),
                    },
                    step=global_step,
                )
...
def main(
    ...
    if train:
        reward_history, training_time = train_sac(args, model_path, jax_device, wandb_enabled)

    if n_eval > 0:
        fig, rmse_pos, episode_rewards, episode_lengths = evaluate_sac(args, n_eval, model_path, render, plot)
