"""PPO training script for reach position task with rotor velocity control.

PPO (Proximal Policy Optimization) implementation for quadrotor position control
using direct RPM setpoints. Environment setup matches TD3 training.
"""
...
class Args:
    """Configuration for PPO training on ReachPos task."""

    seed: int = 42
    """seed of the experiment"""
    jax_device: str = "gpu"
    """environment device"""
    exp_name: str = "ppo_rprv"
    """the name of the experiment"""
    wandb_project_name: str = "rl-ppo-rprv"
    ...
    # Envs & Wrapper settings (from TD3)
...
def make_jitted_envs(num_envs: int, jax_device: str, args: Args, reset_rotor: bool = True) -> ReachPosEnv:
    """Create wrapped environments for PPO training (same as TD3)."""
...
def train_ppo(args: Args, model_path: Path, jax_device: str, wandb_enabled: bool = False) -> None:
    """Train PPO agent on ReachPos task."""
    # setup training
    ...
    # make envs
    envs = make_jitted_envs(num_envs=args.num_envs, jax_device=jax_device, args=args, reset_rotor=True)
...
def main(
    ...
    if train:
        reward_history, training_time = train_ppo(args, model_path, jax_device, wandb_enabled)
