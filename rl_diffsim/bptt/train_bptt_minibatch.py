...
def update_policy(
    ...
    def policy_loss_fn(
        ...
        losses = jp.sum(data.losses, axis=0)
        return losses / (args.num_steps), (
            envs,
            data,
            next_obs,
            next_done,
    ...
    policy_jacobian_fn = jax.jacrev(policy_loss_fn, argnums=(1,), has_aux=True)
    (g_actor,), (envs, data, next_obs, next_done, sum_rewards, key, p_loss) = policy_jacobian_fn(
        envs, agent.actor_states.params, next_obs, next_done, sum_rewards, key
    )
    print(g_actor)
    # g_actor = jax.tree_util.tree_map(lambda x: jp.mean(x, axis=0), g_actor)
    ...
    actor_states, _ = jax.lax.scan(grad_update_step, actor_states, g_actor)
    agent = agent.replace(actor_states=actor_states)
...
def train_bptt(args: Args, model_path: Path, jax_device: str, wandb_enabled: bool = False) -> None:
    ...
    if args.anneal_actor_lr:
        ...
        # actor_lr = optax.exponential_decay(
        #     init_value=args.actor_lr, transition_steps=args.num_iterations, decay_rate=0.994
        # )
    else:
