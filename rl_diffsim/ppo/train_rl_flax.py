...
class ActorNet(nn.Module):
    ...
    def __call__(self, obs: Array) -> tuple[Array, Array]:
        ...
        # Actor mean
        x = obs
        x = nn.Dense(self.hidden_size, kernel_init=self.kernel_init(), bias_init=self.bias_init)(x)
        x = nn.tanh(x)
        x = nn.Dense(self.hidden_size, kernel_init=self.kernel_init(), bias_init=self.bias_init)(x)
        x = nn.tanh(x)
        mean = nn.Dense(self.num_outputs, kernel_init=self.kernel_init(0.01), bias_init=self.bias_init)(x)
        mean = nn.tanh(mean)
        # Actor logstd
        ...
        logstd = jp.broadcast_to(actor_logstd, (mean.shape[0], self.num_outputs))
...
class CriticNet(nn.Module):
    ...
    def __call__(self, obs: Array) -> Array:
        ...
        x = nn.Dense(self.hidden_size, kernel_init=self.kernel_init(), bias_init=self.bias_init)(x)
        x = nn.tanh(x)
        value = nn.Dense(1, kernel_init=self.kernel_init(0.01), bias_init=self.bias_init)(x)
        return value.squeeze(-1)

class Agent(nn.Module):
    """Class defining the agent with actor and critic networks."""
    hidden_size: int = 64
    num_outputs: int = 4

    def setup(self):
        """Setup actor and critic networks."""
        self.actor = ActorNet(hidden_size=self.hidden_size, num_outputs=self.num_outputs)
        self.critic = CriticNet(hidden_size=self.hidden_size)

    def get_action_and_value(self, obs: Array, rng: Array) -> tuple[Array, Array, Array, Array]:
        """Get action, logprob, entropy and value from observation."""
        mean, logstd = self.actor(obs)
        std = jp.exp(logstd)
        dist = jp.random.normal(rng, mean.shape) * std + mean
        action = dist
        logprob = -0.5 * (((action - mean) / (std + 1e-8)) ** 2 + 2 * logstd + jp.log(2 * jp.pi))
        logprob = jp.sum(logprob, axis=-1)
        value = self.critic(obs)
        return action, logprob, dist, value
